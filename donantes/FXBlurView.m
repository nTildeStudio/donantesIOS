...
- (void)setUp
{
    if (!_iterationsSet) _iterations = 1; //JL 3
    if (!_blurRadiusSet) [self blurLayer].blurRadius = 10; //JL 40
...
- (void)setLayerContents:(UIImage *)image
{
    ...
    self.layer.contentsScale = image.scale; //FIXME quizás se tenga que poner 1.0 para pantallas no retina
}
...
- (void)updateAsynchronously:(BOOL)async imagen_origen:(UIImage*)imagen_origen completion:(void (^)())completion
{
    if ([self shouldUpdate])
    {
        UIImage *snapshot = imagen_origen;
